...
// Variável global para armazenar o ID do chamado atual da ordem de serviço
let currentOrdemServicoChamadoId = null;

// Abrir modal da ordem de serviço
async function abrirOrdemServico(chamadoId) {
    if (!chamadoId) {
        exibirMensagem('Selecione um chamado primeiro', 'erro');
        return;
    }
    ...
    try {
        currentOrdemServicoChamadoId = chamadoId;

        // Mostrar loading no modal
        const modalContent = document.getElementById('ordemServicoContent');
        modalContent.innerHTML = `
            <div class="text-center">
                <div class="spinner-border text-primary" role="status">
                    <span class="visually-hidden">Carregando...</span>
                </div>
                <p class="mt-2">Carregando ordem de serviço...</p>
            </div>
        `;

        // Abrir modal
        const modal = new bootstrap.Modal(document.getElementById('ordemServicoModal'));
        modal.show();

        // Buscar dados da ordem de serviço
        const response = await fetch(`/chamados/${chamadoId}/ordem-servico`);
        const data = await response.json();

        if (response.ok) {
            renderizarOrdemServico(data);
        } else {
            modalContent.innerHTML = `
                <div class="alert alert-danger">
                    <i class="bi bi-exclamation-triangle"></i>
                    ${data.erro || 'Erro ao carregar ordem de serviço'}
                </div>
            `;
        }
    } catch (error) {
...
// Renderizar conteúdo da ordem de serviço
function renderizarOrdemServico(data) {
    const modalContent = document.getElementById('ordemServicoContent');

    // Formatar endereço
    const enderecoParts = [];
    if (data.cliente.endereco.rua) enderecoParts.push(data.cliente.endereco.rua);
    if (data.cliente.endereco.numero) enderecoParts.push(data.cliente.endereco.numero);
    if (data.cliente.endereco.complemento) enderecoParts.push(data.cliente.endereco.complemento);
    if (data.cliente.endereco.bairro) enderecoParts.push(data.cliente.endereco.bairro);
    if (data.cliente.endereco.cidade) enderecoParts.push(data.cliente.endereco.cidade);
    if (data.cliente.endereco.estado) enderecoParts.push(data.cliente.endereco.estado);
    if (data.cliente.endereco.cep) enderecoParts.push(`CEP: ${data.cliente.endereco.cep}`);
    ...
    modalContent.innerHTML = `
        <div class="ordem-servico-container">
            <!-- Cabeçalho -->
            <div class="ordem-servico-header">
                <div class="text-center mb-4">
                    <h2 class="text-primary mb-2">
                        <i class="bi bi-file-text"></i> ORDEM DE SERVIÇO
                    </h2>
                    <p class="text-muted">Protocolo: ${data.chamado.protocolo}</p>
                    <p class="text-muted">Data de Geração: ${new Date().toLocaleDateString('pt-BR')} ${new Date().toLocaleTimeString('pt-BR')}</p>
                </div>
            </div>
            ...
            <div class="ordem-servico-section">
                <h4 class="section-title">
                    <i class="bi bi-folder2-open"></i> DADOS DO CHAMADO
                </h4>
                <div class="row">
                    <div class="col-md-6">
                        <p><strong>Protocolo:</strong> ${data.chamado.protocolo}</p>
                        <p><strong>Assunto:</strong> ${data.chamado.assunto}</p>
                        <p><strong>Status:</strong> 
                            <span class="badge bg-${data.chamado.status === 'Aberto' ? 'warning' : 'success'}">${data.chamado.status}</span>
                        </p>
                    </div>
                    <div class="col-md-6">
                        <p><strong>Data de Abertura:</strong> ${formatarData(data.chamado.data_abertura)}</p>
                        ${data.chamado.data_fechamento ? `<p><strong>Data de Fechamento:</strong> ${formatarData(data.chamado.data_fechamento)}</p>` : ''}
                    </div>
                </div>
                ${data.chamado.descricao ? `<p><strong>Descrição:</strong> ${data.chamado.descricao}</p>` : ''}
            </div>
            ...
            <div class="ordem-servico-section">
                ...
                <div class="row">
                    <div class="col-md-6">
                        ...
                        ${data.cliente.nome_fantasia ? `<p><strong>Nome Fantasia:</strong> ${data.cliente.nome_fantasia}</p>` : ''}
                        <p><strong>Email:</strong> ${data.cliente.email || 'Não informado'}</p>
                        <p><strong>Telefone:</strong> ${data.cliente.telefone || 'Não informado'}</p>
                    </div>
                    <div class="col-md-6">
                        <p><strong>Tipo:</strong> ${data.cliente.tipo_cliente || 'Não informado'}</p>
                        ${data.cliente.cnpj_cpf ? `<p><strong>CNPJ/CPF:</strong> ${data.cliente.cnpj_cpf}</p>` : ''}
            ...
            <div class="ordem-servico-section">
                ...
                <div class="row">
                    <div class="col-md-6">
                        <p><strong>Data Agendada:</strong> ${formatarData(data.agendamento.data_agendamento)}</p>
                        ${data.agendamento.data_final_agendamento ? `<p><strong>Data Final:</strong> ${formatarData(data.agendamento.data_final_agendamento)}</p>` : ''}
                    </div>
                    <div class="col-md-6">
                        <p><strong>Status:</strong> 
                            <span class="badge bg-${data.agendamento.status === 'Agendado' ? 'info' : data.agendamento.status === 'Finalizado' ? 'success' : 'secondary'}">${data.agendamento.status}</span>
                        </p>
                        ${data.agendamento.observacoes ? `<p><strong>Observações:</strong> ${data.agendamento.observacoes}</p>` : ''}
                    </div>
                </div>
            </div>
            ` : ''}
            ...
            <div class="ordem-servico-section">
                <h4 class="section-title">
                    <i class="bi bi-clock-history"></i> HISTÓRICO DE ANDAMENTOS
                </h4>
                <div class="andamentos-timeline">
                    ${data.andamentos.length > 0 ? data.andamentos.map(andamento => `
                        <div class="andamento-item">
                            <div class="andamento-header">
                                <span class="andamento-date">
                                    <i class="bi bi-clock"></i> ${formatarData(andamento.data_hora)}
                                </span>
                                ${andamento.username ? `<span class="andamento-user text-muted ms-2"><i class="bi bi-person"></i> ${andamento.username}</span>` : ''}
                            </div>
                            <div class="andamento-content">
                                ${andamento.texto}
                            </div>
                        </div>
                    `).join('') : '<p class="text-muted">Nenhum andamento registrado.</p>'}
                </div>
            </div>

            <!-- Relatório da Visita (se finalizado) -->
            ${data.chamado.status === 'Finalizado' && data.andamentos.length > 0 ? `
            <div class="ordem-servico-section">
                <h4 class="section-title">
                    <i class="bi bi-check-circle"></i> RELATÓRIO DA VISITA TÉCNICA
                </h4>
                <div class="relatorio-visita">
                    ${data.andamentos[data.andamentos.length - 1].texto}
                </div>
                ${data.usuario_finalizacao ? `<p class="text-muted mt-2"><small>Técnico responsável: ${data.usuario_finalizacao}</small></p>` : ''}
            </div>
            ` : ''}
        </div>
    `;
...
function imprimirOrdemServico() {
    const conteudo = document.getElementById('ordemServicoContent').innerHTML;
    const janela = window.open('', '_blank');
    janela.document.write(`
        <html>
            <head>
                <title>Ordem de Serviço - ${currentOrdemServicoChamadoId}</title>
                ...
                <style>
                    @media print {
                        ...
                        .ordem-servico-container { margin: 20px; }
...
async function exportarPDF() {
    if (!currentOrdemServicoChamadoId) {
    ...
    try {
        ...
        const response = await fetch(`/chamados/${currentOrdemServicoChamadoId}/ordem-servico/pdf`);
        ...
        if (response.ok) {
            ...
            a.href = url;
            a.download = `ordem-servico-${currentOrdemServicoChamadoId}.pdf`;
            a.click();
            window.URL.revokeObjectURL(url);
            exibirMensagem('PDF gerado com sucesso!', 'sucesso');
        } else {
            const data = await response.json();
            exibirMensagem(data.erro || 'Erro ao gerar PDF', 'erro');
        }
    } catch (error) {
...
async function exportarImagem() {
    ...
    try {
        ...
        const btn = event.target;
        const originalText = btn.innerHTML;
        btn.innerHTML = '<i class="bi bi-hourglass-split"></i> Gerando imagem...';
        btn.disabled = true;

        const response = await fetch(`/chamados/${currentOrdemServicoChamadoId}/ordem-servico/imagem`);
        ...
        if (response.ok) {
            ...
            const a = document.createElement('a');
            a.href = url;
            a.download = `ordem-servico-${currentOrdemServicoChamadoId}.png`;
            a.click();
            window.URL.revokeObjectURL(url);
            exibirMensagem('Imagem gerada com sucesso!', 'sucesso');
        } else {
            const data = await response.json();
            exibirMensagem(data.erro || 'Erro ao gerar imagem', 'erro');
        }
    } catch (error) {
        console.error('Erro ao gerar imagem:', error);
        exibirMensagem('Erro ao gerar imagem', 'erro');
    } finally {
        // Restaurar botão
        const btn = event.target;
        btn.innerHTML = '<i class="bi bi-image"></i> Exportar JPG';
        btn.disabled = false;
    }
} 