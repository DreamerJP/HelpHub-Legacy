...
def normalizar_sqlite(texto):
    """Função para ser usada no SQLite"""
    if not texto:
        return ""
...
app = Flask(__name__)
CORS(
    app, allow_private_network=False
)  # Desativa explicitamente o cabeçalho Access-Control-Allow-Private-Network
...
def get_or_generate_secret_key():
    ...
    new_key = secrets.token_hex(
        32
    )  # Gera 256 bits (32 bytes) de dados aleatórios hexadecimais
    ...
    try:
        with get_db_connection() as conn:
            ...
            if "user_id" in session:
                cursor.execute(
                    "SELECT id FROM usuarios WHERE id = ?", (session["user_id"],)
                )
            ...
            missing_tables = [
                table for table in critical_tables if table not in table_names
            ]
            if missing_tables:
...
def validate_database_integrity():
    ...
    if "user_id" in session:
        ...
        if current_hash == "USER_DELETED":
            app_logger.warning(
                f"Usuário {session.get('username', 'desconhecido')} foi excluído do banco. Sessão invalidada."
            )
            session.clear()
            _db_hash = None
            return False
        ...
        if current_hash == "STRUCTURE_CHANGED":
            app_logger.warning(
                f"Estrutura do banco de dados foi alterada. Sessão invalidada para usuário {session.get('username', 'desconhecido')}"
            )
            session.clear()
            _db_hash = None
            return False
        ...
        if current_hash != _db_hash:
            app_logger.warning(
                f"Detectada troca do arquivo do banco de dados. Sessão invalidada para usuário {session.get('username', 'desconhecido')}"
            )
            session.clear()
            _db_hash = None
            return False
...
def before_request():
    ...
    if not validate_database_integrity():
        ...
        if (
            ...
            or request.path.startswith("/clientes")
            or request.path.startswith("/chamados")
            or request.path.startswith("/agendamentos")
            or request.path.startswith("/usuarios")
            or request.path.startswith("/system/")
            or request.path.startswith("/admin/")
            or request.path.startswith("/estatisticas")
        ):
            return (
                jsonify(
                    {
...
def auth_login():
    try:
        ...
        if not username or not password:
            auth_logger.warning(
                f"Tentativa de login sem usuário ou senha - IP: {client_ip}"
            )
            return (
                jsonify(
        ...
        with get_db_connection() as conn:
            ...
            if not usuario:
                auth_logger.warning(
                    f"Usuário não encontrado: {username} - IP: {client_ip}"
                )
                return (
                    jsonify({"success": False, "error": "Credenciais inválidas"}),
                    401,
                )
            ...
            if usuario[2] is None and not usuario[4]:
                ...
                if username == "admin" and password == "admin":
                    ...
                    return jsonify(
                        {"success": False, "initial_password_required": True}
                    )
                return (
                    jsonify({"success": False, "error": "Credenciais inválidas"}),
                    401,
            ...
            if usuario[2] and check_password_hash(usuario[2], password):
                ...
                auth_logger.info(
                    f"Login bem-sucedido para o usuário {username} - IP: {client_ip}"
                )
                ...
                arquivos_hoje = glob.glob(
                    os.path.join(BACKUP_DIR, f"backup_{hoje}_*.db")
                )
                app_logger.info(
                    f"Verificação de backup: encontrados {len(arquivos_hoje)} backup(s) para hoje ({hoje})"
                )

                if not arquivos_hoje:
                    # Nenhum backup feito hoje, então realiza o backup
                    app_logger.info(
                        "Nenhum backup encontrado para hoje, realizando backup diário..."
                    )
                    sucesso, mensagem = realizar_backup_diario()
                    backup_info = {"realizado": sucesso, "mensagem": mensagem}
                    ...
                    app_logger.info(
                        f"Verificação de backup após login: já realizado hoje"
                    )

                return jsonify(
                    {
                        "success": True,
                        "user": {"username": usuario[1], "role": usuario[3]},
                        "backup": backup_info,
                    }
                )
            else:
                auth_logger.warning(
                    f"Senha incorreta para usuário {username} - IP: {client_ip}"
                )
                return (
                    jsonify({"success": False, "error": "Credenciais inválidas"}),
                    401,
...
def get_db_connection():
    ...
    try:
        ...
        # Registra a função customizada NORMALIZAR
        conn.create_function("NORMALIZAR", 1, normalizar_sqlite)
    ...
    def wrapper(*args, **kwargs):
        for attempt in range(MAX_RETRIES):
            try:
                ...
                app_logger.warning(
                    f"Operação no banco de dados falhou, tentando novamente... ({attempt + 1}/{MAX_RETRIES})"
                )
...
def criar_tabelas():
    with get_db_connection() as conn:
        ...
        cursor.execute(
            """CREATE TABLE IF NOT EXISTS chamados_andamentos (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
        ...
        # Tabela de associação usuário-departamento
        cursor.execute(
        ...
        if not cursor.fetchone():
            ...
            cursor.execute(
                ...
                (
                    "backup_dir",
                    caminho_padrao,
                    "Diretório para armazenar backups do sistema",
                ),
            )
        ...
        if cursor.fetchone()[0] == 0:
            ...
            cursor.execute(
                "INSERT INTO usuarios (username, password, role, senha_inicial_definida) VALUES (?, NULL, ?, ?)",
                ("admin", "admin", False),
            )
        ...
        if cursor.fetchone()[0] == 0:
            cursor.execute(
                "ALTER TABLE usuarios ADD COLUMN senha_inicial_definida BOOLEAN DEFAULT TRUE"
            )
            # Define como TRUE para usuários existentes
            cursor.execute(
                "UPDATE usuarios SET senha_inicial_definida = TRUE WHERE password IS NOT NULL"
            )
...
def realizar_backup_diario():
    ...
    try:
        ...
        if len(todos_backups) >= 14:
            backups_para_remover = todos_backups[
                :-13
            ]  # Mantém apenas os 13 mais recentes + o novo = 14 total
            app_logger.info(
                f"Removendo {len(backups_para_remover)} backup(s) antigo(s) para manter limite de 14"
            )
            ...
            app_logger.info(
                f"Mantendo todos os {len(todos_backups)} backup(s) existente(s)"
            )
...
def serve_html(filename):
    return send_from_directory(
        os.path.abspath(os.path.join(BASE_DIR, "../HTML")), filename
    )
...
def favicon():
    return send_from_directory(
        os.path.abspath(os.path.join(BASE_DIR, "../IMAGENS")), "favicon.ico"
    )
...
def serve_css(filename):
    return send_from_directory(
        os.path.abspath(os.path.join(BASE_DIR, "../CSS")), filename
    )
...
def serve_js(filename):
    return send_from_directory(
        os.path.abspath(os.path.join(BASE_DIR, "../JS")), filename
    )
...
def serve_imagens(filename):
    return send_from_directory(
        os.path.abspath(os.path.join(BASE_DIR, "../IMAGENS")), filename
    )
...
def serve_docs(filename):
    return send_from_directory(
        os.path.abspath(os.path.join(BASE_DIR, "../DOCS")), filename
    )
...
def friendly_page_p(page):
    if page in PAGE_MAPPING:
        ...
        return send_from_directory(
            os.path.abspath(os.path.join(BASE_DIR, "../HTML")), html_file
        )
    else:
        abort(404)
...
def index():
    ...
    if "user_id" not in session:
        return redirect("/p/login")
    return send_from_directory(
        os.path.abspath(os.path.join(BASE_DIR, "../HTML")), "02-home.html"
    )
...
def help_page():
    return send_from_directory(
        os.path.abspath(os.path.join(BASE_DIR, "../HTML")), "09-help.html"
    )
...
def snake_game():
    ...
    return send_from_directory(
        os.path.abspath(os.path.join(BASE_DIR, "../HTML")), "10-snake.html"
    )
...
def serve_markdown(topic):
    ...
    try:
        return send_from_directory(
            os.path.abspath(os.path.join(BASE_DIR, "../DOCS")), file_path
        )
...
def listar_clientes():
    try:
        ...
        with get_db_connection() as conn:
            ...
            app_logger.info(
                f"Listando clientes - Página: {pagina}, Limite: {limite}, Total: {total}"
            )
...
def cadastrar_cliente():
    try:
        ...
        with get_db_connection() as conn:
            ...
            return (
                jsonify(
                    {"mensagem": "Cliente cadastrado com sucesso!", "id": cliente_id}
                ),
                201,
            )
    except Exception as e:
...
def buscar_clientes():
    try:
        ...
        termo_normalizado = normalizar_sqlite(termo)

        # Query SQL para buscar clientes com base no termo de pesquisa, ignorando acentos
        query = """
        SELECT id, nome, nome_fantasia, email, telefone, ativo,
                tipo_cliente, cnpj_cpf, ie_rg, contribuinte_icms, rg_orgao_emissor,
                nacionalidade, naturalidade, estado_nascimento, data_nascimento,
                sexo, profissao, estado_civil, inscricao_municipal
        FROM clientes
        WHERE NORMALIZAR(CAST(id AS TEXT)) LIKE ? 
            OR NORMALIZAR(nome) LIKE ? 
            OR NORMALIZAR(email) LIKE ?
        """
        ...
        with get_db_connection() as conn:
            ...
            search_term = f"%{termo_normalizado}%"
...
def obter_notas_cliente(id):
    try:
        with get_db_connection() as conn:
            cursor = conn.cursor()
            cursor.execute(
                "SELECT notas FROM notas_clientes WHERE cliente_id = ?", (id,)
            )
...
def salvar_notas_cliente(id):
    try:
        ...
        with get_db_connection() as conn:
            ...
            if not cursor.fetchone():
                return (
                    jsonify({"success": False, "error": "Cliente não encontrado"}),
                    404,
                )
            ...
            if nota_existente:
                ...
                if not notas.strip():
                    cursor.execute(
                        "DELETE FROM notas_clientes WHERE cliente_id = ?", (id,)
                    )
                    app_logger.info(f"Notas excluídas para o cliente {id}")
                else:
                    # Atualiza as notas existentes
                    cursor.execute(
                        "UPDATE notas_clientes SET notas = ? WHERE cliente_id = ?",
                        (notas, id),
                    )
                ...
                if notas.strip():
                    cursor.execute(
                        "INSERT INTO notas_clientes (cliente_id, notas) VALUES (?, ?)",
                        (id, notas),
                    )
                    app_logger.info(f"Notas adicionadas para o cliente {id}")
            conn.commit()
            return jsonify(
                {"success": True, "message": "Notas atualizadas com sucesso"}
            )
    except Exception as e:
...
def abrir_chamado():
    try:
        ...
        with get_db_connection() as conn:
            ...
            # Verifica se o cliente existe
            cursor.execute(
                "SELECT id FROM clientes WHERE id = ?", (dados["cliente_id"],)
            )
            if not cursor.fetchone():
                app_logger.warning(
                    f"Cliente não encontrado ao abrir chamado: {dados['cliente_id']}"
                )
                return jsonify({"erro": "Cliente não encontrado"}), 404
            ...
            cursor.execute(
                ...
                INSERT INTO chamados (
                    ...
                    assunto, telefone, solicitante, departamento_id
                )
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            """,
                (
                    ...
                    dados.get("departamento_id", None),
                ),
            )
    ...
    try:
        ...
        with get_db_connection() as conn:
            ...
            for chamado in chamados:
                chamado_dict = list(chamado)
                if chamado[10]:
                    chamado_dict[10] = chamado[10]
                else:
                    chamado_dict[10] = "Cliente removido"
                chamados_processados.append(chamado_dict)

            # Registra operação no log
            app_logger.info(
                f"Listando chamados - Status: {status}, Página: {pagina}, Limite: {limite}, Total: {total}"
            )
...
def editar_chamado(id):
    try:
        dados = sanitize_input(request.json or {})  # Sanitiza os dados recebidos
        app_logger.debug(
            f"Dados recebidos para edição do chamado {id}: {dados}"
        )  # Log dos dados recebidos

        # Validação do tamanho do assunto
        if (
            isinstance(dados, dict)
            and isinstance(dados.get("assunto"), str)
            and len(dados.get("assunto", "")) > 70
        ):
            return jsonify({"erro": "O assunto deve ter no máximo 70 caracteres"}), 400
        ...
        with get_db_connection() as conn:
            ...
            for campo, coluna in campos_permitidos.items():
                if isinstance(dados, dict) and campo in dados:
                    ...
                    app_logger.debug(
                        f"Campo {campo} será atualizado para: {dados.get(campo)}"
                    )
            ...
            if not campos_atualizacao:
                app_logger.warning(
                    f"Nenhum campo válido para atualização no chamado {id}"
                )
...
def excluir_chamado(id):
    try:
        with get_db_connection() as conn:
            ...
            cursor.execute(
                "SELECT COUNT(*) FROM agendamentos WHERE chamado_id = ?", (id,)
            )
            agendamentos_count = cursor.fetchone()[0]

            if agendamentos_count > 0:
                app_logger.warning(
                    f"Tentativa de excluir chamado {id} com agendamentos associados"
                )
                return (
                    jsonify(
                        {
...
def adicionar_andamento(chamado_id):
    try:
        ...
        with get_db_connection() as conn:
            ...
            if not cursor.fetchone():
                app_logger.warning(
                    f"Chamado não encontrado ao adicionar andamento: {chamado_id}"
                )
                return jsonify({"erro": "Chamado não encontrado"}), 404
            ...
            app_logger.debug(
                f"Inserindo andamento: chamado_id={chamado_id}, data_hora={data_hora}, texto={texto}"
            )
            ...
            cursor.execute(
                "SELECT id, data_hora, texto FROM chamados_andamentos WHERE id = ?",
                (novo_id,),
            )
...
def excluir_andamento(andamento_id):
    try:
        with get_db_connection() as conn:
            ...
            cursor.execute(
                "DELETE FROM chamados_andamentos WHERE id=?", (andamento_id,)
            )

            # Verifica se a entrada de progresso foi encontrada
            if cursor.rowcount == 0:
                app_logger.warning(
                    f"Andamento não encontrado para exclusão: {andamento_id}"
                )
        ...
        with get_db_connection() as conn:
            ...
            cursor.execute(
                ...
                SELECT ch.id, ch.cliente_id, ch.descricao, ch.status, ch.data_abertura, 
                    ...
                    cl.nome as cliente_nome, cl.telefone as cliente_telefone, d.nome as departamento_nome, ch.departamento_id
                FROM chamados ch
                LEFT JOIN clientes cl ON ch.cliente_id = cl.id
                LEFT JOIN departamentos d ON ch.departamento_id = d.id
                WHERE ch.id = ?
            """,
                (id,),
            )
            ...
            agendamento_data = (
                dict(
                    zip(
                        ["data_agendamento", "data_final_agendamento", "observacoes"],
                        agendamento,
                    )
                )
                if agendamento
            ...
            return jsonify(
                {
                    ...
                    "departamento_nome": chamado[12] or "-",
                    "departamento_id": chamado[13],
                    "andamentos": [
                        {
                            "id": a["id"],
...
def buscar_chamados():
    try:
        ...
        with get_db_connection() as conn:
            ...
            cursor.execute(
                query, (status, search_term, search_term, search_term, search_term)
            )
            chamados = cursor.fetchall()

            # Registra operação no log
            app_logger.info(
                f"Busca de chamados realizada - Status: {status}, Termo: {termo}"
            )

            # Retorna os chamados encontrados
            return jsonify({"chamados": chamados, "total": len(chamados)})
    except Exception as e:
        # Registra falha no log
...
def obter_estatisticas():
    try:
        ...
        # Só aplica filtro se for inteiro válido
        if departamento_id not in (None, "", "null"):
            try:
        ...
        with get_db_connection() as conn:
            ...
            dep_params = []
            if departamento_id:
                dep_filter = " AND departamento_id = ?"
                dep_params = [departamento_id]

            if periodo == "total":
                cursor.execute(
                    f"SELECT COUNT(*) FROM chamados WHERE status = 'Aberto' {dep_filter}",
                    dep_params,
                )
                chamados_abertos = cursor.fetchone()[0] or 0

                cursor.execute(
                    f"SELECT COUNT(*) FROM chamados WHERE status = 'Finalizado' {dep_filter}",
                    dep_params,
                )
                chamados_fechados = cursor.fetchone()[0] or 0

                cursor.execute(
                    f"SELECT MIN(date(data_abertura)), MAX(date(data_abertura)) FROM chamados {(f'WHERE departamento_id = ?' if departamento_id else '')}",
                    dep_params,
                )
                ...
                if data_min and data_max:
                    ...
                    cursor.execute(
                        f"SELECT COUNT(*) FROM chamados {(f'WHERE departamento_id = ?' if departamento_id else '')}",
                        dep_params,
                    )
                    total_chamados = cursor.fetchone()[0] or 0
                ...
                if departamento_id:
                    if query_filtro:
                        query_filtro += " AND departamento_id = ?"
                    else:
                        query_filtro = "WHERE departamento_id = ?"
                    params.append(departamento_id)

                cursor.execute(
                    f"SELECT COUNT(*) FROM chamados {query_filtro} AND status = 'Aberto'",
                    params,
                )
                chamados_abertos = cursor.fetchone()[0] or 0

                cursor.execute(
                    f"SELECT COUNT(*) FROM chamados {query_filtro} AND status = 'Finalizado'",
                    params,
                )
                chamados_fechados = cursor.fetchone()[0] or 0

                cursor.execute(f"SELECT COUNT(*) FROM chamados {query_filtro}", params)
                total_chamados_periodo = cursor.fetchone()[0] or 0
                ...
                media_diaria = round(total_chamados_periodo / dias, 1) if dias else 0

            # Obter os últimos 5 chamados (filtrar por departamento se houver)
            ultimos_query = """
            ...
            ultimos_params = []
            if departamento_id:
                ultimos_query += " WHERE c.departamento_id = ?"
                ultimos_params.append(departamento_id)
            ultimos_query += " ORDER BY c.data_abertura DESC LIMIT 5"
            cursor.execute(ultimos_query, ultimos_params)
            ultimos_chamados = [
                {
                    "id": row[0],
                    "assunto": row[1] or "",
                    "data_abertura": row[2] or "",
                    "status": row[3] or "",
    ...
    try:
        ...
        with get_db_connection() as conn:
            ...
            if not result or result[0]:
                ...
                return (
                    jsonify(
                        {"success": False, "error": "Senha inicial já foi definida"}
                    ),
                    403,
                )
...
def check_role():
    ...
    app_logger.info(
        f"Verificando papel do usuário: {session.get('username', 'desconhecido')}, papel: {session.get('role', 'desconhecido')}"
    )
...
def renew_session():
    ...
    client_ip = get_real_ip()
    app_logger.warning(
        f"Tentativa de renovar sessão sem user_id na sessão - IP: {client_ip}"
    )
...
def check_session():
    ...
    try:
        with get_db_connection() as conn:
            ...
            if not usuario:
                app_logger.warning(
                    f"Usuário {session.get('username', 'desconhecido')} não encontrado no banco. Sessão invalidada."
                )
                session.clear()
                return (
                    jsonify(
...
def check_first_access():
    try:
        with get_db_connection() as conn:
            ...
            return jsonify({"is_first_access": is_first_access})
...
def obter_info_backups():
    try:
        ...
        for arquivo in todos_backups:
            ...
            data_criacao = datetime.fromtimestamp(os.path.getctime(arquivo)).strftime(
                "%Y-%m-%d %H:%M:%S"
            )
...
def configurar_backup():
    ...
    if request.method == "GET":
        try:
            with get_db_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(
                    'SELECT valor FROM configuracoes WHERE chave = "backup_dir"'
                )
                resultado = cursor.fetchone()
            ...
            return (
                jsonify(
                    {"success": False, "error": f"Erro ao obter configuração: {str(e)}"}
                ),
                500,
            )
    ...
    try:
        ...
        return (
            jsonify(
                {"success": False, "error": f"Erro ao atualizar configuração: {str(e)}"}
            ),
            500,
        )
...
def realizar_backup_manual():
    ...
    try:
        ...
        # Retorna o resultado
        return jsonify({"success": sucesso, "mensagem": mensagem})
    except Exception as e:
        app_logger.error(f"Erro ao realizar backup manual: {str(e)}")
        return (
            jsonify(
                {"success": False, "error": f"Erro ao realizar backup manual: {str(e)}"}
            ),
            500,
        )
...
def listar_usuarios():
    try:
        if session.get("role") != "admin":
            app_logger.warning(
                f"Usuário {session.get('username', 'desconhecido')} tentou listar usuários sem permissão."
            )
        ...
        with get_db_connection() as conn:
            ...
            # Registra sucesso no log
            app_logger.info(
                f"Usuário {session.get('username', 'desconhecido')} listou usuários com sucesso."
            )
...
def criar_usuario():
    if session.get("role") != "admin":
        app_logger.warning(
            f"Usuário {session.get('username', 'desconhecido')} tentou criar usuário sem permissão."
        )
    ...
    try:
        ...
        with get_db_connection() as conn:
            ...
            # Registra sucesso no log
            app_logger.info(
                f"Usuário {session.get('username', 'desconhecido')} criou usuário {username} com papel {role}."
            )
        ...
        # Registra falha no log
        app_logger.warning(
            f"Tentativa de criar usuário com username já existente: {username}"
        )
...
def atualizar_usuario(id):
    if session.get("role") != "admin":
        app_logger.warning(
            f"Usuário {session.get('username', 'desconhecido')} tentou atualizar usuário sem permissão."
        )
        return jsonify({"error": "Unauthorized"}), 403
    ...
    try:
        ...
        with get_db_connection() as conn:
            ...
            if not current_user:
                app_logger.warning(
                    f"Tentativa de atualizar usuário com ID inexistente: {id}"
                )
            ...
            if current_user[0] == "admin":
                ...
                if username != "admin" or role != "admin":
                    app_logger.warning(
                        "Tentativa de modificar username ou role do admin."
                    )
                    return (
                        jsonify({"error": "Cannot modify admin username or role"}),
                        403,
                    )

                if not password:
                    app_logger.warning(
                        "Tentativa de alterar senha do admin sem fornecer nova senha."
                    )
                    return jsonify({"error": "Password is required for admin"}), 400

                hashed_password = generate_password_hash(password)
                cursor.execute(
                    "UPDATE usuarios SET password=? WHERE id=?", (hashed_password, id)
                )
                app_logger.info(
                    f"Senha do admin alterada por {session.get('username', 'desconhecido')}."
                )
            else:
                ...
                app_logger.info(
                    f"Usuário {username} atualizado por {session.get('username', 'desconhecido')}."
                )
        ...
        # Registra falha no log
        app_logger.warning(
            f"Tentativa de atualizar usuário para username já existente: {username}"
        )
...
def excluir_usuario(id):
    if session.get("role") != "admin":
        app_logger.warning(
            f"Usuário {session.get('username', 'desconhecido')} tentou excluir usuário sem permissão."
        )
        return jsonify({"error": "Unauthorized"}), 403
    ...
    try:
        with get_db_connection() as conn:
            ...
            if id == session.get("user_id"):
                app_logger.warning(
                    f"Usuário {session.get('username', 'desconhecido')} tentou se autoexcluir."
                )
            ...
            # Registra sucesso no log
            app_logger.info(
                f"Usuário {id} excluído por {session.get('username', 'desconhecido')}."
            )
...
def obter_usuario(id):
    if session.get("role") != "admin":
        app_logger.warning(
            f"Usuário {session.get('username', 'desconhecido')} tentou obter detalhes de usuário sem permissão."
        )
        return jsonify({"error": "Unauthorized"}), 403
    ...
    try:
        with get_db_connection() as conn:
            ...
            cursor.execute(
                "SELECT id, username, role, created_at FROM usuarios WHERE id = ?",
                (id,),
            )
            usuario = cursor.fetchone()
...
@app.route("/usuarios/<int:user_id>/departamentos", methods=["PUT"])
@login_required
def atualizar_departamentos_usuario(user_id):
    if session.get("role") != "admin":
        app_logger.warning(
            f"Usuário {session.get('username', 'desconhecido')} tentou atualizar departamentos de usuário sem permissão."
        )
        return jsonify({"error": "Unauthorized"}), 403
    try:
        data = request.json or {}
        ...
        with get_db_connection() as conn:
            cursor = conn.cursor()
            cursor.execute(
                "DELETE FROM usuario_departamento WHERE usuario_id=?", (user_id,)
            )
            for dep_id in departamentos:
                cursor.execute(
            ...
            conn.commit()
        app_logger.info(
            f"Departamentos do usuário {user_id} atualizados por {session.get('username', 'desconhecido')}."
        )
        return jsonify({"message": "Departamentos atualizados com sucesso"})
    except Exception as e:
        app_logger.error(f"Erro ao atualizar departamentos do usuário: {e}")
        return jsonify({"error": "Erro ao atualizar departamentos do usuário"}), 500


@app.route("/usuarios/<int:user_id>/departamentos", methods=["GET"])
@login_required
def listar_departamentos_usuario(user_id):
    if session.get("role") != "admin":
        app_logger.warning(
            f"Usuário {session.get('username', 'desconhecido')} tentou listar departamentos de usuário sem permissão."
        )
        return jsonify({"error": "Unauthorized"}), 403
    try:
        with get_db_connection() as conn:
            ...
            cursor.execute(
                """
                SELECT d.id, d.nome, d.descricao FROM departamentos d
                JOIN usuario_departamento ud ON d.id = ud.departamento_id
                WHERE ud.usuario_id=?
            """,
                (user_id,),
            )
            deps = cursor.fetchall()
        return jsonify([{"id": d[0], "nome": d[1], "descricao": d[2]} for d in deps])
    except Exception as e:
        app_logger.error(f"Erro ao listar departamentos do usuário: {e}")
        return jsonify({"error": "Erro ao listar departamentos do usuário"}), 500
...
def listar_departamentos():
    try:
        with get_db_connection() as conn:
            cursor = conn.cursor()
            cursor.execute(
                "SELECT id, nome, descricao FROM departamentos ORDER BY id DESC"
            )
            departamentos = cursor.fetchall()
        return jsonify(
            [{"id": d[0], "nome": d[1], "descricao": d[2]} for d in departamentos]
        )
    except Exception as e:
        app_logger.error(f"Erro ao listar departamentos: {e}")
...
def criar_departamento():
    if session.get("role") != "admin":
        app_logger.warning(
            f"Usuário {session.get('username', 'desconhecido')} tentou criar departamento sem permissão."
        )
        return jsonify({"error": "Unauthorized"}), 403
    try:
        data = request.json or {}
        ...
        app_logger.info(
            f"Departamento '{nome}' criado por {session.get('username', 'desconhecido')}."
        )
        return jsonify({"id": dep_id, "nome": nome, "descricao": descricao}), 201
    except Exception as e:
        app_logger.error(f"Erro ao criar departamento: {e}")
        return jsonify({"error": "Erro ao criar departamento"}), 500


@app.route("/departamentos/<int:dep_id>", methods=["PUT"])
@login_required
def editar_departamento(dep_id):
    if session.get("role") != "admin":
        app_logger.warning(
            f"Usuário {session.get('username', 'desconhecido')} tentou editar departamento sem permissão."
        )
        return jsonify({"error": "Unauthorized"}), 403
    try:
        data = request.json or {}
        ...
        with get_db_connection() as conn:
            ...
            cursor.execute(
                ...
                (nome, descricao, dep_id),
            )
            conn.commit()
        app_logger.info(
            f"Departamento {dep_id} editado por {session.get('username', 'desconhecido')}."
        )
        return jsonify({"id": dep_id, "nome": nome, "descricao": descricao})
    except Exception as e:
        app_logger.error(f"Erro ao editar departamento: {e}")
        return jsonify({"error": "Erro ao editar departamento"}), 500


@app.route("/departamentos/<int:dep_id>", methods=["DELETE"])
@login_required
def excluir_departamento(dep_id):
    if session.get("role") != "admin":
        app_logger.warning(
            f"Usuário {session.get('username', 'desconhecido')} tentou excluir departamento sem permissão."
        )
        return jsonify({"error": "Unauthorized"}), 403
    try:
        with get_db_connection() as conn:
            ...
            conn.commit()
        app_logger.info(
            f"Departamento {dep_id} excluído por {session.get('username', 'desconhecido')}."
        )
        return jsonify({"message": "Departamento excluído com sucesso"})
    except Exception as e:
        app_logger.error(f"Erro ao excluir departamento: {e}")
        return jsonify({"error": "Erro ao excluir departamento"}), 500


@app.route("/departamentos/<int:dep_id>/usuarios", methods=["GET"])
@login_required
def listar_usuarios_departamento(dep_id):
    if session.get("role") != "admin":
        app_logger.warning(
            f"Usuário {session.get('username', 'desconhecido')} tentou listar usuários de departamento sem permissão."
        )
        return jsonify({"error": "Unauthorized"}), 403
    try:
        with get_db_connection() as conn:
            ...
            cursor.execute(
                """
                SELECT u.id, u.username, u.role, u.created_at FROM usuarios u
        ...
        app_logger.error(f"Erro ao listar usuários do departamento: {e}")
        return jsonify({"error": "Erro ao listar usuários do departamento"}), 500
    ...
    try:
        ...
        with get_db_connection() as conn:
            ...
            if agendamento_existente:
                app_logger.warning(
                    f"Agendamento conflitante encontrado para o chamado {chamado_id}"
                )
                return (
                    jsonify(
                        {
            ...
            # Registra sucesso no log
            app_logger.info(
                f"Agendamento criado com sucesso para o chamado {chamado_id}"
            )
                ...
                agendamentos_list.append(
                    {
                        ...
                        "departamento_nome": agendamento[11] or "-",
                    }
                )
...
def excluir_agendamento(id):
    try:
        with get_db_connection() as conn:
            ...
            if cursor.rowcount == 0:
                app_logger.warning(
                    f"Tentativa de excluir agendamento inexistente com ID: {id}"
                )
...
def atualizar_agendamento(id):
    try:
        ...
        if not data_final_agendamento:
            try:
                ...
                data_inicial = datetime.fromisoformat(
                    data_agendamento.replace("Z", "+00:00")
                )
                ...
                try:
                    ...
                    data_inicial = datetime.strptime(
                        data_agendamento, "%Y-%m-%dT%H:%M:%S%z"
                    )
                    data_final = data_inicial + timedelta(hours=1)
                    data_final_agendamento = data_final.isoformat()
        ...
        with get_db_connection() as conn:
            ...
            # Monta e executa a query
            query = (
                f"UPDATE agendamentos SET {', '.join(campos_atualizacao)} WHERE id=?"
            )
            cursor.execute(query, valores)
            ...
            if cursor.rowcount == 0:
                app_logger.warning(
                    f"Tentativa de atualizar agendamento inexistente com ID: {id}"
                )
                return jsonify({"erro": "Agendamento não encontrado"}), 404
    ...
    try:
        with get_db_connection() as conn:
            ...
            resultado = {
                ...
                "departamento_nome": agendamento[12] or "-",
            }
...
def finalizar_ordem_servico(chamado_id):
    try:
        ...
        app_logger.info(
            f"Relatório extraído: '{relatorio_visita}', comprimento: {len(relatorio_visita) if relatorio_visita else 0}"
        )
        ...
        with get_db_connection() as conn:
            ...
            if not cursor.fetchone():
                app_logger.warning(
                    f"Chamado não encontrado ao finalizar ordem: {chamado_id}"
                )
                return jsonify({"erro": "Chamado não encontrado"}), 404
            ...
            app_logger.info(
                f"Andamento {novo_andamento_id} criado com sucesso para chamado {chamado_id}"
            )
            ...
            # Registra sucesso no log
            app_logger.info(
                f"Ordem de serviço finalizada com sucesso para o chamado {chamado_id}"
            )
        ...
        with get_db_connection() as conn:
            ...
            cursor.execute(
                ...
                SELECT 
                    ...
                    c.solicitante, d.nome as departamento_nome
                FROM chamados c
                LEFT JOIN clientes cl ON c.cliente_id = cl.id
                LEFT JOIN departamentos d ON c.departamento_id = d.id
                WHERE c.id = ?
            ...
            ordem_servico = {
                "chamado": {
                    ...
                    "solicitante": chamado_data[22] or "-",
                    "departamento_nome": chamado_data[23] or "-",
                },
                "cliente": {
                    "id": chamado_data[7],
                    "nome": chamado_data[8],
                ...
                "andamentos": [
                    {"data_hora": andamento[0], "texto": andamento[1]}
            ...
            app_logger.info(
                f"Dados da ordem de serviço obtidos com sucesso para chamado {chamado_id}"
            )
            return jsonify(ordem_servico)

    except Exception as e:
        app_logger.error(
            f"Erro ao obter dados da ordem de serviço: {str(e)}", exc_info=True
        )
        return (
            jsonify({"erro": f"Erro ao obter dados da ordem de serviço: {str(e)}"}),
            500,
        )
...
def gerar_pdf_ordem_servico(chamado_id):
    ...
    try:
        ...
        story.append(
            Paragraph(f"<b>Data de Abertura:</b> {chamado_data[5]}", normal_style)
        )
        if chamado_data[6]:
            story.append(
                Paragraph(f"<b>Data de Fechamento:</b> {chamado_data[6]}", normal_style)
            )
        story.append(Spacer(1, 20))
        ...
        if chamado_data[9]:
            story.append(
                Paragraph(f"<b>Nome Fantasia:</b> {chamado_data[9]}", normal_style)
            )
        ...
        if chamado_data[13]:
            story.append(
                Paragraph(f"<b>CNPJ/CPF:</b> {chamado_data[13]}", normal_style)
            )
        ...
        if agendamento:
            ...
            story.append(
                Paragraph(f"<b>Data Agendada:</b> {agendamento[1]}", normal_style)
            )
            if agendamento[2]:
                story.append(
                    Paragraph(f"<b>Data Final:</b> {agendamento[2]}", normal_style)
                )
            if agendamento[3]:
                story.append(
                    Paragraph(f"<b>Observações:</b> {agendamento[3]}", normal_style)
                )
        ...
        story.append(
            Paragraph(f"<b>Solicitante:</b> {chamado_data[22] or '-'}", normal_style)
        )
        story.append(
            Paragraph(f"<b>Departamento:</b> {chamado_data[23] or '-'}", normal_style)
        )
        story.append(
            Paragraph(f"<b>Data de Abertura:</b> {chamado_data[5]}", normal_style)
        )
        ...
        app_logger.info(
            f"PDF da ordem de serviço gerado com sucesso para chamado {chamado_id}"
        )
        ...
        app_logger.error(
            f"Erro ao gerar PDF da ordem de serviço: {str(e)}", exc_info=True
        )
...
def table_data(table_name):
    ...
    try:
        ...
        with get_db_connection() as conn:
            ...
            app_logger.info(
                f"Administrador {session.get('username')} visualizou dados da tabela {table_name}"
            )
...
def import_table_data(table_name):
    ...
    try:
        ...
        if (
            not dados
            or not isinstance(dados.get("data"), list)
            or not dados.get("columns")
        ):
            return (
                jsonify({"success": False, "error": "Dados de importação inválidos"}),
                400,
            )
        ...
        with get_db_connection() as conn:
            ...
            if not cursor.fetchone():
                return (
                    jsonify({"success": False, "error": "Tabela não encontrada"}),
                    404,
                )
            ...
            if import_mode == "replace":
                try:
                    ...
                    app_logger.warning(
                        f"Administrador {session.get('username')} excluiu todos os registros da tabela {table_name} para importação"
                    )
                except Exception as e:
                    return (
                        jsonify(
                            {
                                "success": False,
                                "error": f"Erro ao limpar tabela: {str(e)}",
                            }
                        ),
                        500,
                    )
            ...
            colunas_validas = [
                col for col in colunas_importacao if col in tabela_colunas
            ]
            ...
            if not colunas_validas:
                return (
                    jsonify(
                        {
                            "success": False,
                            "error": "Nenhuma coluna válida encontrada para importação",
                        }
                    ),
                    400,
                )
            ...
            for registro in dados["data"]:
                try:
                    ...
                    app_logger.error(
                        f"Erro ao importar registro para tabela {table_name}: {e}"
                    )
                    registros_falhos += 1
            ...
            # Registrar no log
            app_logger.info(
                f"Administrador {session.get('username')} importou {registros_importados} registros para a tabela {table_name}"
            )
...
def export_table(format, table_name):
    ...
    try:
        ...
        prefixo = ""
        if table_name in ordem_importacao:
            prefixo = f"{ordem_importacao.index(table_name)+1}_"
        ...
        with get_db_connection() as conn:
            ...
            if format.lower() == "xlsx":
                ...
                return send_file(
                    ...
                    as_attachment=True,
                    download_name=f"{prefixo}{table_name}_export.xlsx",
                )
                ...
                return send_file(
                    ...
                    mimetype="text/csv",
                    as_attachment=True,
                    download_name=f"{prefixo}{table_name}_export.csv",
                )
            else:
